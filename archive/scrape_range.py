...
# ── קריאת הגדרות ──────────────────────────────────────────
_cfg = json.loads(Path("config_scrape_range.json").read_text(encoding="utf-8"))
...
PROGRESS_FILE = OUTPUT_DIR / "progress_range.json"
DATA_FILE     = OUTPUT_DIR / "data.json"
LOG_FILE      = OUTPUT_DIR / "log_range.txt"
...
async def download_word_for_row(page, row_idx: int, case_number: str, file_date: str) -> Path | None:
    global _download_count

    date_dir = OUTPUT_DIR / file_date
    ...
    try:
        ...
        ts = datetime.now().strftime("%H%M%S")
        fname = f"{safe_cn}_{file_date}_{ts}.docx"
...
async def process_results(page):
    ...
    while True:
        ...
        for i, row in enumerate(rows):
            ...
            save_path = await download_word_for_row(page, i, case_number, file_date)
...
async def main():
    ...
    log("=" * 60)
    log(f"scrape_range — {DATE_FROM} עד {DATE_TO}")
        ...
        try:
            ...
            for dt_name in DECISION_TYPES:
                ...
                for court_idx in range(1, num_courts):
                    ...
                    await scrape_range(page, court_idx, dt_name, from_d, to_d)
