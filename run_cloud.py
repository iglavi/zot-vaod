"""
גרסת ענן - מריצים headless על Linux
"""
...
OUTPUT_DIR = Path("/home/user/zot-vaod/output")
COURT_INDEX = 1
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        for method, locator in [
            ("button", page.locator("button", has_text="אישור")),
            ("input", page.locator("input[value='אישור']")),
        ]:
            if not dismissed:
                try:
        ...
        if doc_count == 0:
            ...
            for kw in ["לא נמצאו", "אין תוצאות", "0 תוצאות"]:
                if kw in body_text:
                    print(f"  נמצא בדף: '{kw}'")
        ...
        if table is None:
            print("  לא מצאתי טבלה עם יותר משורה אחת")
            await browser.close()
            return
        ...
        if total_data_rows == 0:
            print("  אין שורות נתונים")
            await browser.close()
            return

        # --- שלב 4: חילוץ שורה ראשונה ---
        print("\n[4] חולץ שורה ראשונה...")
