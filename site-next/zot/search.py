...
# מוסדות צבאיים/ביטחוניים - כל אחד שם מדויק (לא תבנית "ב<עיר>" כמו שאר
# הסוגים), ולכן בלי הרשימה הזו כל אחד מהם היה הופך לסוג-ערכאה נפרד משלו
# (ראו split_court) במקום קיבוץ הגיוני אחד "צבאי" בתפריט הסינון.
_MILITARY_COURTS = {
    "בית דין צבאי דרום וז\"י",
    "בית דין צבאי מטכ\"ל ועורף",
    "בית דין צבאי מטכל ועורף",
    "בית דין צבאי מרכז וח\"א",
    "בית דין צבאי צפון וח\"י",
...
def split_court(court: str) -> tuple[str, str]:
    ...
    (ראו _MILITARY_COURTS) מקובצים יחד תחת סוג אחד "צבאי" במקום זאת."""
    s = (court or "").strip()
    s = _STRAY_PREFIX_RE.sub("", s).strip()
    s = _STRAY_BET_PREFIX_RE.sub("", s).strip()
    if not s:
        return "", ""
    if s in _MILITARY_COURTS:
        return "צבאי", ""
    if s in _PAROLE_COMMITTEES:
        return "ועדת שחרורים", ""
