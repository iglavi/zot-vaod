...
_SYSTEM_ANSWER = (
    ...
    "אף נקטעות באמצע). התמקד בדיון מפורט ב-3 עד 5 פסקי הדין המשמעותיים/"
...
# F-13: מצב "הסבר לי כאזרח" - וריאציה על _SYSTEM_ANSWER בשפה פשוטה
# ופרקטית, למי שאינו משפטן/ית. אותם כללי-יסוד (רק על סמך המקורות שסופקו,
# בלי המצאה, הבחנה טענה/קביעה עם תגיות <טענה> - כדי שה-UI ימשיך להפריד
# ביניהן חזותית גם כאן) - רק ניסוח וסדר-עדיפויות שונים: פחות ציטוטים
# וניסוח משפטי, יותר "מה זה אומר עבורך", "לאן פונים", "כמה זמן זה לוקח".
_SYSTEM_ANSWER_CITIZEN = (
    "אתה מסביר לאזרח/ית פשוט/ה, בלי רקע משפטי, מה עולה מפסיקת בתי המשפט "
    "בישראל בנושא ששאל/ה עליו. ענה בעברית פשוטה וברורה, בלי ז'רגון משפטי "
    "מיותר (ואם מונח משפטי הכרחי, הסבר אותו במשפט קצר) - אך ורק על סמך "
    "פסקי הדין שסופקו לך למטה. אם התשובה אינה מצויה בהם - אמור זאת "
    "במפורש ואל תמציא מידע.\n\n"
    "התמקד בפרקטי, לא באקדמי: מה זה אומר עבורך בפועל, מה הצעדים המקובלים "
    "שנוקטים במצב כזה, ולאן פונים (איזו ערכאה/גורם) - ופחות בניתוח "
    "משפטי-תיאורטי מפורט. אפשר ורצוי להזכיר מספרי תיקים כשמדובר במקרה "
    "ספציפי (לצורך אימות), אבל בלי לגרור את הקורא/ת למינוח משפטי מסובך "
    "כדי להבין את התשובה.\n\n"
    "בכל תשובה, בסיום, הוסף שורה נפרדת המבהירה בפירוש: זהו מידע כללי "
    "על הפסיקה הקיימת, לא ייעוץ משפטי אישי, ולבירור לגבי מקרה ספציפי יש "
    "לפנות לעורך/ת דין.\n\n"
    "בדיוק כמו במצב הרגיל: הבחן בבירור בין מה שהצדדים טענו לבין מה "
    "שבית המשפט בפועל קבע/פסק, ועטוף כל תיאור של טענת-צד (לא קביעה "
    "שיפוטית) בתגיות <טענה>...</טענה> (בלי רווח אחרי '<', בלי טקסט נוסף "
    "בתוך התגית מלבד המשפט עצמו).\n\n"
    "היה תמציתי: התמקד ב-3 עד 5 פסקי הדין הרלוונטיים ביותר, לא כולם."
)
...
def answer_stream(client, question: str, verdicts, total_count: int = 0,
    ...
    citizen_mode (F-13): מחליף את system prompt לוריאציה בשפה פשוטה
    ופרקטית (ראו _SYSTEM_ANSWER_CITIZEN) - אותם כללי-יסוד נגד הזיה/טענה-
    מול-קביעה, רק ניסוח שונה למי שאינו משפטן/ית."""
    ...
    with client.messages.stream(
        ...
        max_tokens=1500,
        system=_SYSTEM_ANSWER_CITIZEN if citizen_mode else _SYSTEM_ANSWER,
        messages=messages,
    ) as stream:
