...
async def scrape_range(page, court_idx: int, dt_name: str,
    ...
    if from_date == to_date:
        if judge_idx is None:
            ...
            # קוראים שופטים לפני PostBack — אחרי do_search ה-dropdown מתאפס
            await navigate_to_search(page)
            await page.locator("#LocateByParameters1_ddlSelectCourt").select_option(index=court_idx)
...
async def scrape_day(pages: list, dt_name: str, day: date,
    ...
    # ── שלב 2: רשת ביטחון — יש 100+, מחלקים לפי ערכאה ───
    n = len(pages)
    log(f"  [{dt_name}] {day_str} — 100+ תוצאות, מחפש לפי ערכאה ({n} טאבים במקביל)...")
    court_indices = list(range(1, num_courts))
    chunk_size = max(1, len(court_indices) // n)
    chunks = [court_indices[i:i + chunk_size] for i in range(0, len(court_indices), chunk_size)]
    # אם יש יותר chunks מ-pages — מאחד את העודפים לתוך האחרון
    while len(chunks) > n:
        chunks[-2].extend(chunks.pop())

    await asyncio.gather(
        *[scrape_courts_subset(pages[i], dt_name, day, court_names, chunks[i], f"טאב {i+1}")
...
async def main():
    ...
    async with async_playwright() as p:
        ...
        # חמישה contexts נפרדים — כל context מחזיק session נפרד בשרת
        NUM_TABS = 10
        contexts = [await browser.new_context(accept_downloads=True, user_agent=ua) for _ in range(NUM_TABS)]
        pages = [await ctx.new_page() for ctx in contexts]
        page = pages[0]  # טאב ראשי (לשימוש כשאין מקביליות)
