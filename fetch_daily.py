...
פרטי הגישה נקראים ממשתני סביבה או מקובץ .env מקומי:
  ...
  DECISIONS_DAYS   — כמה תיקיות תאריך אחרונות לבדוק (ברירת מחדל: 7; 0 = כל הקיימות)
...
def main() -> int:
    ...
    # גדל ללא גבול עם גיל הארכיון). נותחו 34+ תיקיות תאריך מההיסטוריה
    # המלאה ב-daily_log.txt (לא רק כמה ימים אחרונים): תיקייה חדשה נפתחת
    # קטנה (500-800 קבצים) ותופחת פי 40-50 עד ההרצה הבאה למחרת, ולא אף
    # פעם יותר מזה — כל תיקייה שנצפתה 3+ פעמים התייצבה בדיוק אחרי מחזור
    # יומי אחד, בלי יוצא מן הכלל. חלון של יומיים כבר היה מספיק לפי הנתונים
    # בפועל; 7 ימים (שבוע) משאיר שוליים נדיבים גם לתרחיש שלא נצפה בנתונים
    # (חג/סופ"ש שמאט את קצב ההעלאה של בתי המשפט), במחיר זניח (7 בקשות
    # רשימת-תיקייה בכל ריצה במקום 44+ וגדל).
    only_days = int(os.environ.get("DECISIONS_DAYS", "7") or "7")
