...
from . import config
from .extract import extract_decision_date, extract_judge, filed_date_from_case, read_text
...
def _index_documents(docs_dir: Path) -> dict[str, Path]:
    ...
    for f in docs_dir.rglob("*"):
        if f.is_file() and f.suffix.lower() in (".docx", ".doc", ".txt", ".pdf"):
...
def build(metadata_path: Path | None = None, docs_dir: Path | None = None,
    ...
    if not metadata_path.exists():
        raise FileNotFoundError(f"לא נמצא קובץ המטא-דאטה: {metadata_path}")
