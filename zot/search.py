...
_COURT_TYPE_PATTERNS = [
    ...
    ("עבודה", re.compile(r"^בית\s*[-–]?\s*ה?דין\s+ה?(?:אזורי|ארצי)\s+לעבודה\s*ב?(.*)$")),
...
def split_court(court: str) -> tuple[str, str]:
    ...
    for type_name, pat in _COURT_TYPE_PATTERNS:
        ...
        if m:
            ...
            # 'ב?' האופציונלי בתבנית 'עבודה' (ראו למעלה) לא יכול להבחין בין
            # שני המקורות ('...לעבודה בבאר שבע' מול '...לעבודה באר שבע')
            # ומקצץ בטעות את ה-ב' האמיתית מהמקור השני. אין דו-משמעות דומה
            # מוכרת בפועל עבור עוד עיר כלשהי במאגר.
            if type_name == "עבודה" and city == "אר שבע":
                city = "באר שבע"
